...
let checkbox = document.getElementById('checkbox-class');
let div_start = document.querySelector('.div-start')
let iconL = document.querySelector('.icon-left')
let iconR = document.querySelector('.icon-right')

 checkbox.addEventListener('change', () => {
        for (let i = 0; i < tanlanganlar.length; i++) {
            let div = document.createElement('div');
            div.classList.add('div-div-start');
            div.innerHTML = `
                <p class="uzbek">${tanlanganlar[i]['name-uzb']}</p>
                <i class="fa-solid fa-arrow-left icon-l"></i>
                <img class="image" src="rasmlar/${tanlanganlar[i]['img']}" alt="ananas">
                <i class="fa-solid fa-arrow-right icon-r"></i>
                <p class="english">${tanlanganlar[i]['name-eng']}</p>
            `;
